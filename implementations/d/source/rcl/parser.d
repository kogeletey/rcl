...
struct Parser {
  ...
  Token peek() {
    auto p = lexer.pos; auto l = lexer.line; auto c = lexer.col;
    auto t = lexer.nextToken();
    lexer.pos = p; lexer.line = l; lexer.col = c;
    return t;
  }
  ...
  AstNode parseValue() {
    if (cur.typ == TokenType.str) {
      auto v = cur.value;
      eat(TokenType.str);
      return AstNode(NodeKind.str, v, 0, false, false, null);
    }
    ...
    if (cur.typ == TokenType.ident) {
      ...
      auto v = t.value;
      eat(TokenType.ident);
      if (v == "true") return AstNode(NodeKind.boolv, "", 0, true, false, null);
      if (v == "false") return AstNode(NodeKind.boolv, "", 0, false, false, null);
      fail("Invalid bare value '" ~ v ~ "'", t);
    }
    if (cur.typ == TokenType.lbrack) return parseArray();
    fail("Unexpected token: " ~ cur.typ.to!string, cur);
    return AstNode(NodeKind.str, "", 0, false, false, null);
  }
  ...
  AstNode parseArray() {
    ...
    return AstNode(NodeKind.arr, "", 0, false, false, items);
  }

  BlockNode parseBlock() {
    BlockNode b; b.name = cur.value; eat(TokenType.ident);
    if (cur.typ == TokenType.str) { b.hasArg = true; b.arg = cur.value; eat(TokenType.str); }
    eat(TokenType.kwDo);
    string[] seen;
    while (cur.typ != TokenType.kwEnd) {
      ...
      auto nxt = peek();
      if (nxt.typ == TokenType.kwDo || nxt.typ == TokenType.str) {
        auto child = parseBlock();
        if (child.hasArg) b.named ~= child; else b.blocks ~= child;
      } else if (nxt.typ == TokenType.eq || nxt.typ == TokenType.dot) {
        auto key = parsePropertyKey(); ensureKey(key, seen); seen ~= key;
        eat(TokenType.eq);
        b.props[key] = parseValue(); b.propOrder ~= key;
